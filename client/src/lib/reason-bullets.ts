...
const SEASON_ACHIEVEMENT_LABELS: Partial<Record<SeasonAchievementId, string>> = {
  ...
  HKMVP: 'MVP',
  HKDefensivePlayer: 'Defensive Player of the Year',
  HKDefensiveForward: 'Defensive Forward of the Year',
  HKGOY: 'Goalie of the Year',
...
function generateSeasonAchievementBullet(player: Player, achievementId: SeasonAchievementId, teams: Team[], constraintLabel?: string, sport?: string): ReasonBullet | null {
  ...
  if (seasons.length === 0) {
    ...
    if ((baseAchievementId.startsWith('Season') || baseAchievementId.startsWith('FBSeason') || baseAchievementId.startsWith('HKSeason')) && (baseAchievementId.includes('PPG') || baseAchievementId.includes('RPG') || baseAchievementId.includes('APG') || baseAchievementId.includes('SPG') || baseAchievementId.includes('BPG') || baseAchievementId.includes('MPG') || baseAchievementId.includes('Points') || baseAchievementId.includes('3PM') || baseAchievementId.includes('Assists') || baseAchievementId.includes('Steals') || baseAchievementId.includes('Blocks') || baseAchievementId.includes('Stocks') || baseAchievementId.includes('PPGRPG') || baseAchievementId.includes('PPGRPGAPG') || baseAchievementId.includes('SPGBPG3PMG') || baseAchievementId.includes('PassYds') || baseAchievementId.includes('RushYds') || baseAchievementId.includes('RecYds') || baseAchievementId.includes('Receptions') || baseAchievementId.includes('Sacks') || baseAchievementId.includes('Tackles') || baseAchievementId.includes('Interceptions') || baseAchievementId.includes('PassTD') || baseAchievementId.includes('RecTD') || baseAchievementId.includes('RushTD') || baseAchievementId.includes('Scrimmage') || baseAchievementId.includes('AllPurpose') || baseAchievementId.includes('TFL') || baseAchievementId.includes('Goals') || baseAchievementId.includes('Shots') || baseAchievementId.includes('Hits') || baseAchievementId.includes('Takeaways') || baseAchievementId.includes('PowerPlay') || baseAchievementId.includes('SHGoals') || baseAchievementId.includes('GWGoals') || baseAchievementId.includes('FaceoffPct') || baseAchievementId.includes('TOI') || baseAchievementId.includes('PIM') || baseAchievementId.includes('SavePct') || baseAchievementId.includes('GAA') || baseAchievementId.includes('Shutouts') || baseAchievementId.includes('Saves') || baseAchievementId.includes('Starts') || baseAchievementId.includes('Plus'))) {
    ...
    const awardNegativeMessages: Record<string, string> = {
      ...
      'HKDefensivePlayer': 'Was never the Defensive Player of the Year',
      'HKDefensiveForward': 'Was never the Defensive Forward of the Year',
      'HKGOY': 'Was never the Goalie of the Year',
...
function generateCareerAchievementBullet(player: Player, achievementId: string, teams: Team[], constraintLabel?: string, sport?: string): ReasonBullet | null {
  ...
  if (draftAchievementIds.includes(baseAchievementId)) {
    ...
    if (statInfo) {
      return {
        text: `${formatNumber(statInfo.value)} Career ${statInfo.label}`.trim(),
        type: 'award'
      };
    } else {
      // Fallback if statInfo is null (e.g., non-statistical career achievement)
...
function getCareerStatInfo(player: Player, achievementId: string): { value: number; label: string } | null {
  const statMap: Record<string, { field: string | string[]; label: string }> = {
    // Basketball
    'careerPoints': { field: 'pts', label: 'Points' },
    'careerRebounds': { field: 'trb', label: 'Rebounds' },
    'careerAssists': { field: 'ast', label: 'Assists' },
    'careerSteals': { field: 'stl', label: 'Steals' },
    'careerBlocks': { field: 'blk', label: 'Blocks' },
    'careerThrees': { field: 'tpm', label: 'Threes Made' },
    
    // Baseball
    'careerHits': { field: 'h', label: 'Hits' },
    'careerHomeRuns': { field: 'hr', label: 'Home Runs' },
    'careerRBIs': { field: 'rbi', label: 'RBIs' },
    'careerStolenBases': { field: 'sb', label: 'Stolen Bases' },
    'careerRuns': { field: 'r', label: 'Runs' },
    'careerWinsP': { field: 'w', label: 'Wins (P)' },
    'careerStrikeouts': { field: 'soPit', label: 'Strikeouts' },
    'careerSavesP': { field: 'sv', label: 'Saves' },
    ...
    // Football
    'careerPassTDs': { field: 'pssTD', label: 'Passing TDs' },
    'careerPassYds': { field: 'pssYds', label: 'Passing Yards' },
    'careerRushYds': { field: 'rusYds', label: 'Rushing Yards' },
    'careerRushTDs': { field: 'rusTD', label: 'Rushing TDs' },
    'careerRecTDs': { field: 'recTD', label: 'Receiving TDs' },
    'careerRecYds': { field: 'recYds', label: 'Receiving Yards' },
    'careerSacks': { field: ['sks', 'defSk'], label: 'Sacks' },
    'careerInterceptions': { field: 'defInt', label: 'Interceptions' },
  };
  ...
  // Map specific career achievement IDs to generic stat IDs
  const careerAchievementIdToGenericStatIdMap: Record<string, string> = {
    'career20kPoints': 'careerPoints',
    'career20000Points': 'careerPoints',
    'career10kRebounds': 'careerRebounds',
    'career10000Rebounds': 'careerRebounds',
    'career5kAssists': 'careerAssists',
    'career5000Assists': 'careerAssists',
    'career2kSteals': 'careerSteals',
    'career2000Steals': 'careerSteals',
    'career1500Blocks': 'careerBlocks',
    'career2kThrees': 'careerThrees',
    'career2000ThreeMade': 'careerThrees',
    'career3000Hits': 'careerHits',
    'career500HRs': 'careerHomeRuns',
    'career1500RBIs': 'careerRBIs',
    'career400SBs': 'careerStolenBases',
    'career1800Runs': 'careerRuns',
    'career300Wins': 'careerWinsP',
    ...
    'career50kPassYds': 'careerPassYds',
    'career12kRushYds': 'careerRushYds',
    'career100RushTDs': 'careerRushTDs',
    'career100RecTDs': 'careerRecTDs',
    'career12kRecYds': 'careerRecYds',
    'career100Sacks': 'careerSacks',
    'career30Ints': 'careerInterceptions',
  };

  const genericStatId = careerAchievementIdToGenericStatIdMap[baseAchievementId] || baseAchievementId;

  const statInfo = statMap[genericStatId];
