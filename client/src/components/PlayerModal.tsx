...
function getAchievementDetails(
  ...
  const getStatValueAndYears = (statField: string, threshold?: number, isAvg = false) => {
    if (!player.stats) return { value: undefined, years: undefined };
    let bestValue = isAvg ? 0 : -Infinity;
    let bestYear = 0;
    const qualifyingSeasons: number[] = [];

    player.stats.filter(s => !s.playoffs && (s.gp || 0) > 0).forEach(s => {
      let statValue = (s as any)[statField] || 0;
      if (isAvg && s.gp && s.gp > 0) {
        statValue = statValue / s.gp;
      }

      if (threshold === undefined || (statValue >= threshold)) {
        qualifyingSeasons.push(s.season);
      }
    ...
    if (qualifyingSeasons.length > 0) {
      const sortedSeasons = qualifyingSeasons.sort((a, b) => a - b);
      years = sortedSeasons.length === 1 ? sortedSeasons[0].toString() : `${sortedSeasons[0]}–${sortedSeasons[sortedSeasons.length - 1]}`;
      isPlural = qualifyingSeasons.length > 1;
    }
    if (bestYear > 0) {
      value = isAvg ? bestValue.toFixed(1) : bestValue.toLocaleString();
    }
    return { value, years };
  };
  ...
  if (achievementId.startsWith('career')) {
    ...
    // Season statistical achievements
    const statMap: Record<string, { field: string; isAverage?: boolean; threshold?: number }> = {
      Season30PPG: { field: 'pts', isAverage: true, threshold: 30 },
      Season2000Points: { field: 'pts', threshold: 2000 },
      Season200_3PM: { field: 'tpm', threshold: 200 },
      Season12RPG: { field: 'trb', isAverage: true, threshold: 12 },
      Season10APG: { field: 'ast', isAverage: true, threshold: 10 },
      Season800Rebounds: { field: 'trb', threshold: 800 },
      Season700Assists: { field: 'ast', threshold: 700 },
      Season2SPG: { field: 'stl', isAverage: true, threshold: 2 },
      Season2_5BPG: { field: 'blk', isAverage: true, threshold: 2.5 },
      Season150Steals: { field: 'stl', threshold: 150 },
      Season150Blocks: { field: 'blk', threshold: 150 },
  ...
  Season3PPercent: { field: 'tpPct', isAverage: true, threshold: 0.4 },
      ...
      Season25_5_5: { field: '25/5/5', threshold: 0 }, // Special handling
      Season20_10_5: { field: '20/10/5', threshold: 0 }, // Special handling
      Season1_1_1: { field: '1/1/1', threshold: 0 }, // Special handling
      // Football
      FBSeason4kPassYds: { field: 'pssYds', threshold: 4000 },
      FBSeason1200RushYds: { field: 'rusYds', threshold: 1200 },
      FBSeason100Receptions: { field: 'rec', threshold: 100 },
      FBSeason15Sacks: { field: 'sks', threshold: 15 },
      FBSeason140Tackles: { field: 'defTck', threshold: 140 },
      FBSeason5Interceptions: { field: 'defInt', threshold: 5 },
      FBSeason30PassTD: { field: 'pssTD', threshold: 30 },
      FBSeason1300RecYds: { field: 'recYds', threshold: 1300 },
      FBSeason10RecTD: { field: 'recTD', threshold: 10 },
      FBSeason12RushTD: { field: 'rusTD', threshold: 12 },
      FBSeason1600Scrimmage: { field: 'scrimmageYds', threshold: 1600 },
      FBSeason2000AllPurpose: { field: 'allPurposeYds', threshold: 2000 },
      FBSeason15TFL: { field: 'defTckLoss', threshold: 15 },
      // Hockey
      HKSeason40Goals: { field: 'goals', threshold: 40 },
      HKSeason60Assists: { field: 'assists', threshold: 60 },
      HKSeason90Points: { field: 'points', threshold: 90 },
      HKSeason25Plus: { field: 'pm', threshold: 25 },
      HKSeason250Shots: { field: 's', threshold: 250 },
      HKSeason150Hits: { field: 'hit', threshold: 150 },
      HKSeason100Blocks: { field: 'blk', threshold: 100 },
      HKSeason60Takeaways: { field: 'tk', threshold: 60 },
      HKSeason20PowerPlay: { field: 'powerPlayPoints', threshold: 20 },
      HKSeason3SHGoals: { field: 'shG', threshold: 3 },
      HKSeason7GWGoals: { field: 'gwG', threshold: 7 },
      HKSeason55FaceoffPct: { field: 'faceoffPct', isAverage: true, threshold: 0.55 },
      HKSeason22TOI: { field: 'toiPerGame', isAverage: true, threshold: 22 },
      HKSeason70PIM: { field: 'pim', threshold: 70 },
      HKSeason920SavePct: { field: 'savePct', isAverage: true, threshold: 0.920 },
      HKSeason260GAA: { field: 'gaaRate', isAverage: true, threshold: 2.60 },
      HKSeason6Shutouts: { field: 'so', threshold: 6 },
      HKSeason2000Saves: { field: 'sv', threshold: 2000 },
      HKSeason60Starts: { field: 'gs', threshold: 60 },
    };

    const statConfig = statMap[achievementId];
    if (statConfig) {
      const { value: statValue, years: statYears } = getStatValueAndYears(statConfig.field, statConfig.threshold, statConfig.isAverage);
      value = statValue;
      years = statYears;
      isAverage = statConfig.isAverage || false; // Set isAverage here
    }
